...
static VOID CopyBits32_32(BLT_INFO* pDst, CONST BLT_INFO* pSrc, UINT NumRects, CONST RECT *pRects)
{
...
static VOID GetPitches(CONST BLT_INFO* pBltInfo, LONG* pPixelPitch, LONG* pRowPitch)
{
...
static BYTE* GetRowStart(CONST BLT_INFO* pBltInfo, CONST RECT* pRect)
{
    BYTE* pRet = NULL;
...
static VOID CopyBitsGeneric(BLT_INFO* pDst, CONST BLT_INFO* pSrc, UINT NumRects, CONST RECT *pRects)
{
...
{
    // source->pBits might be coming from user-mode. User-mode addresses when accessed by kernel need to be protected by a __try/__except.
    ...
    __try
    {
        if (dest->BitsPerPel == 32 &&
            source->BitsPerPel == 32 &&
            dest->Rotation == D3DKMDT_VPPR_IDENTITY &&
            source->Rotation == D3DKMDT_VPPR_IDENTITY)
        {
            // This is by far the most common copy function being called
            CopyBits32_32(dest, source, numRects, rects);
        }
        else
        {
            CopyBitsGeneric(dest, source, numRects, rects);
        }
    }
    ...
    {
       //"Either dst (0x%p) or src (0x%p) bits encountered exception during access.", dest->pBits, source->pBits
    }
}
